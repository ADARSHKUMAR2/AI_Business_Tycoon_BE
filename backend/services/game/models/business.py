...
class Business(BaseModel):
    ...
    price_multiplier:  float                    = Field(1.0, gt=0, le=3.0,
                                                        description="Global price multiplier")
    ...
    def calculate_store_rating(self) -> float:
        """
        Compute store rating from current trash count.
        Rating starts at 5.0 and drops by 0.1 per trash item, floored at 0.0.
        """
        penalty = len(self.trash_items) * 0.1
        return round(max(0.0, 5.0 - penalty), 2)

    def recalculate_rating(self) -> None:
        """Recalculate and store the live store rating. Call after any trash change."""
        self.store_rating = self.calculate_store_rating()
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                ...
                "inventory":        {
                    "rice": {
                        "name": "Rice (1kg)", "cost": 40.0, "price": 50.0,
                        "stock": 8, "max_stock": 10, "total_sold": 0
                    }
                },
                "employees":        [],
                "trash_items":      [],
                ...
                "stats": {
                    ...
                    "total_customers_served": 0, "average_transaction": 0.0
                },
