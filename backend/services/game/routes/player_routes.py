...
async def create_player(player_data: PlayerCreate):
    """Create a new player."""
    return await PlayerController.create_player(player_data)

@router.get("/list", response_model=List[str])
async def get_all_players():
    """List all player IDs."""
    return await PlayerController.get_all_players()
...
async def get_player(player_id: str = Path(..., description="Player ID")):
    ...
    return await PlayerController.get_player(player_id)
...
async def update_player(
    ...
    return await PlayerController.update_player(player_id, update_data)

@router.delete("/{player_id}")
async def delete_player(player_id: str = Path(..., description="Player ID")):
    """Delete player data."""
    return await PlayerController.delete_player(player_id)
